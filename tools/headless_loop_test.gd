...
func _smoke_case(failed: PackedStringArray) -> void:
	...
	_expect(failed, not early.ok, "S1 rival smoke refused off-turn")
	...
	_expect(failed, snap.uav_remaining() == 1, "S1 UAV charge untouched")
	_expect(failed, snap.decoy_available(), "S1 decoy charge untouched")
	_expect(failed, snap.you_marks() == before, "S1 no Marks spend")
	_expect(failed, server.account_marks == before, "S1 ledger unchanged")
	_expect(failed, int(snap.you_exposure()) == 50, "S1 exposure unchanged")
	_expect(failed, not snap.last_action().has("hitChance"), "S1 result has no hitChance")
	_expect(failed, not snap.last_action().has("highGroundApplied"), "S1 result has no highGroundApplied")
	r = server.apply_action(mid, pid_a, ActionIntent.smoke())
	_expect(failed, not r.ok, "S1 second smoke refused")
	...
	_expect(failed, snap.smoke_active(), "S2 still live on the enemy turn")
	_expect(failed, not snap.smoke_available(), "S2 still spent")
	r = server.apply_action(mid, pid_b, ActionIntent.attack(int(open_a["q"]), int(open_a["r"])))
	_expect(failed, r.ok, "S3 occupy while smoke is live")
	var last: Dictionary = r.snapshot.get("lastAction", {})
	_expect(failed, bool(last.get("hit", false)), "S3 occupy hits")
	_expect(failed, is_equal_approx(float(last.get("hitChance", -1.0)), Contract.BASE_HIT_CHANCE), "S3 no Attack +0.10 from smoke")
	_expect(failed, last.get("highGroundApplied") == false, "S3 highGroundApplied false")
	_expect(failed, Contract.HIGH_GROUND_HIT == 0.10, "S3 HIGH GROUND table unchanged")
	...
	open_a = server.find_hex_of_type(mid, Contract.TYPE_OPEN)
	open_b = server.find_hex_of_type(mid, Contract.TYPE_OPEN, open_a)
	var hard: Dictionary = server.find_hex_of_type(mid, Contract.TYPE_HARD, open_b)
	_expect(failed, not hard.is_empty(), "S4 board has HARD")
	server.apply_action(mid, pid_a, ActionIntent.select_hex(int(hard["q"]), int(hard["r"])))
	server.apply_action(mid, pid_b, ActionIntent.select_hex(int(open_b["q"]), int(open_b["r"])))
	server.apply_action(mid, pid_a, ActionIntent.start())
	_expect(failed, Snapshot.from_dict(server.get_snapshot(mid, pid_a)).you_high_ground_active(), "S4 real HARD lights HG before smoke")
	r = server.apply_action(mid, pid_a, ActionIntent.smoke())
	snap = Snapshot.from_dict(r.snapshot)
	_expect(failed, snap.you_high_ground_active(), "S4 real HARD stays HG")
	_expect(failed, snap.smoke_active(), "S4 smoke still active on HARD")
	r = server.apply_action(mid, pid_a, ActionIntent.end_turn(50))
	r = server.apply_action(mid, pid_b, ActionIntent.recon(1, 1))
	r = server.apply_action(mid, pid_b, ActionIntent.end_turn(50))
	snap = Snapshot.from_dict(server.get_snapshot(mid, pid_a))
	_expect(failed, not snap.smoke_active(), "S2 expired after the enemy end_turn")
	_expect(failed, not snap.smoke_available(), "S2 charge stays spent")
	_expect(failed, snap.you_high_ground_active(), "S4 HG still the real HARD hex")
	r = server.apply_action(mid, pid_a, ActionIntent.smoke())
	_expect(failed, not r.ok, "S2 smoke_spent after the enemy turn")
	...
	_expect(failed, board.smoke_tint_active(), "S5 soft tint while smokeActive")
	board.apply_snapshot(Snapshot.from_dict({
		"status": Contract.STATUS_ACTIVE,
		"you": {"hex": {"q": 2, "r": 2}, "highGroundActive": true},
	}))
	_expect(failed, not board.smoke_tint_active(), "S5 missing smokeActive does not tint")
	board.free()
