	...
	if unk_img != null:
		for y in unk_img.get_height():
			for x in unk_img.get_width():
				...
				if px.a > 0.8 and px.r > 0.82 and px.g > 0.82 and px.b > 0.75:
					mark_y += float(y)
					mark_top = mini(mark_top, y)
					mark_bot = maxi(mark_bot, y)
	if unk_img != null and mark_n > 0:
		mark_dx = mark_x / float(mark_n) - float(unk_img.get_width()) * 0.5
		mark_dy = mark_y / float(mark_n) - float(unk_img.get_height()) * 0.5
...
func _exposure_floor_case(failed: PackedStringArray) -> void:
	...
	_expect(failed, session.exposure_floor == 40, "shop omit keeps the players floor")
...
func _operative_xp_plate_case(failed: PackedStringArray) -> void:
	...
	_expect(failed, Contract.next_exposure_floor_tip(1) == "Exposure floor · L5 · 40%", "L1 next floor tip")
	_expect(failed, Contract.next_exposure_floor_tip(4) == Contract.EXPOSURE_NEXT_TIP_L5, "L4 next floor tip")
	_expect(failed, Contract.next_exposure_floor_tip(5) == "Exposure floor · L10 · 30%", "L5 next floor tip")
	_expect(failed, Contract.next_exposure_floor_tip(9) == Contract.EXPOSURE_NEXT_TIP_L10, "L9 still names L10")
	_expect(failed, Contract.next_exposure_floor_tip(10) == "Exposure floor · L15 · 20%", "L10 next floor tip")
	_expect(failed, Contract.next_exposure_floor_tip(14) == Contract.EXPOSURE_NEXT_TIP_L15, "L14 still names L15")
	_expect(failed, Contract.next_exposure_floor_tip(15) == "", "L15 omits the next floor tip")
	_expect(failed, Contract.next_exposure_floor_tip(0) == "" and Contract.next_exposure_floor_tip(40) == "", "out of band omits the tip")
	...
	_expect(failed, plate.floor_tip_visible() and plate.floor_tip_text() == Contract.EXPOSURE_NEXT_TIP_L5, "L1 names the L5 floor")
	...
	_expect(failed, plate.floor_tip_text() == "Exposure floor · L5 · 40%", "L4 floor tip")
	...
	_expect(failed, plate.floor_tip_text() == "Exposure floor · L10 · 30%", "L5 floor tip")
	...
	_expect(failed, plate.floor_tip_text() == Contract.EXPOSURE_NEXT_TIP_L10, "L8 still names L10")
	plate.bind(true, 10, true, 964)
	_expect(failed, plate.level_text() == "L10" and not plate.tip_visible(), "L10 drops the smoke tip")
	_expect(failed, plate.floor_tip_text() == "Exposure floor · L15 · 20%", "L10 floor tip")
	plate.bind(true, 14, true, 1355)
	_expect(failed, plate.floor_tip_text() == Contract.EXPOSURE_NEXT_TIP_L15, "L14 still names L15")
	plate.bind(true, 15, true, 1400)
	_expect(failed, plate.visible and plate.level_text() == "L15" and not plate.floor_tip_visible() and plate.floor_tip_text() == "", "L15 omits the floor tip")
	_expect(failed, not plate.tip_visible(), "L15 also omits the smoke tip")
	plate.bind(false, 9, true, 450)
	_expect(failed, not plate.visible and plate.level_text() == "" and not plate.tip_visible() and plate.progress() == -1, "xp alone hides the plate")
	_expect(failed, not plate.floor_tip_visible() and plate.floor_tip_text() == "", "missing operativeLevel omits the floor tip")
	plate.bind(true, 2, false, 0)
	_expect(failed, not plate.visible and plate.level_text() == "" and not plate.tip_visible() and plate.progress() == -1, "level alone hides the plate")
	_expect(failed, plate.floor_tip_text() == "", "a hidden plate does not keep the floor tip")
	...
	session.apply_shop({"you": {"marks": 9, "xp": 40}})
	_expect(failed, session.xp == 40 and session.operative_level == 3, "shop xp without a level keeps the fallback level")
	_expect(failed, session.operative_level != Contract.operative_level_from_xp(40), "xp does not replace operativeLevel")
	_expect(failed, session.exposure_floor == 40, "shop omit keeps the ShopYou floor")
	session.apply_shop({"you": {"marks": 9, "operativeLevel": 4}})
	_expect(failed, session.operative_level == 4 and session.xp == 40, "shop level without xp does not invent a total")
	...
	_expect(failed, plate_src.find("exposureFloor") < 0, "plate script does not read the live floor")
	_expect(failed, plate_src.find("next_exposure_floor_tip") >= 0, "plate binds the level tip")
	_expect(failed, plate_src.find("ClientSession") < 0, "plate script does not name the session autoload")
	var match_dir := DirAccess.open("res://scenes/match")
	_expect(failed, match_dir != null, "match scenes are readable")
	if match_dir != null:
		match_dir.list_dir_begin()
		var match_name := match_dir.get_next()
		while match_name != "":
			if not match_dir.current_is_dir() and match_name.ends_with(".gd"):
				var match_body := FileAccess.get_file_as_string("res://scenes/match/%s" % match_name)
				_expect(failed, match_body.find("Exposure floor ·") < 0, "H2 %s has no hideout floor tip" % match_name)
				_expect(failed, match_body.find("next_exposure_floor_tip") < 0, "H2 %s does not bind the hideout tip" % match_name)
			match_name = match_dir.get_next()
		match_dir.list_dir_end()
	var lobby_src := FileAccess.get_file_as_string("res://scenes/lobby/hideout_lobby.gd")
	_expect(failed, lobby_src.find("func _bind_shop_me()") >= 0, "hideout binds GET /shop/me")
	_expect(failed, lobby_src.find("MatchAPI.get_shop_me()") >= 0, "shop/me is the hideout call")
	var me_fn := _fn_body(lobby_src, "_bind_shop_me")
	_expect(failed, me_fn.find("use_live_api") < 0, "shop/me bind is not live-only")
	server.clear_all()
