...
GLASSLINE_API_BASE=https://glassline-api.vercel.app python3 tools/live_brush_cover_smoke.py  # B1–B6 BRUSH cover; PENDING if coverApplied missing
...
| POST | `/matches/:id/abandon` | mid-match leave + join Bearer, **no body** → same forfeit path as 30s silence (`endReason: forfeit`, Marks +15/0). `ready`/`waiting` → 409 `match_not_active`. Already `ended` → 409 `match_already_ended` (no second grant). |
...
The same plate can show a one-shot **terrain coach** chip when a hard hex lights HIGH GROUND, and another when leafy BRUSH softens a shot. Each dismisses forever (`coachTerrainSeen`). The board stays live. Notes: `COACH_TERRAIN_NOTES.md`.

Hideout **GEAR** is a wood strip (top right): a speaker chip toggles the same mute as match **SOUND / MUTE** (`user://glassline_settings.json`), and **RESET TIPS** asks first, then clears `coachSeen` and `coachTerrainSeen`. No API / Marks / account. Notes: `SETTINGS_GEAR_NOTES.md`.
