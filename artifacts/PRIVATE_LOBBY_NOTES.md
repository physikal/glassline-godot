...
`python3 tools/live_lobby_smoke.py` prefers LIVE once create is 200/201. Bare 404 (no `lobby_not_found`) → `LIVE_LOBBY_PENDING`. Log: `artifacts/live_lobby_smoke.txt`.

## Gates P1–P6

| Gate | Result | Evidence |
| --- | --- | --- |
| **P1** Create / wait | mock + LIVE | `waiting`, `lob_` id, 6-char, no match yet, Marks frozen |
| **P2** Join code → seated | mock + LIVE | Guest seat B; `ready` + `matchId` + per-seat `joinToken`. GET keeps lobby snap. |
| **P3** Same rules / Marks / rematch | mock + LIVE | Miss no invented Hot. Kill ★25 / ★3. Rematch still offered. |
| **P4** Bad / expired code | mock + LIVE | `invalid_lobby_code` / `lobby_not_found` / `already_in_lobby` / TTL. Field stays editable. |
| **P5** Cancel / leave → hideout | mock + LIVE | Waiting `cancelled`, wallet frozen, no match. Ready → `lobby_already_started`. No A4. |
