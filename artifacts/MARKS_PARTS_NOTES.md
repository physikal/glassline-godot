...
ARMORY third row under guns: the same PARTS wood plate, three slots, T1 then T2 on each slot. Wood chips, toy glyphs, ★ prices. T2 is the higher ★ on that slot (`OPTIC T2` ★150, `STOCK T2` ★175, `BARREL T2` ★200), not a new shop dialect. No hit% / spot% / exposure on the row. Each PARTS row paints a peg beside the toy glyph: unowned T1/T2 use the dim locked peg, owned and equipped stay the bright green pegs. No gold EQUIPPED / OWNED / LOCKED strip. Equipped slots also paint a green peg chip on the rack (the worn id, so T2 replaces the T1 chip) and a glyph peg on the exposure doll. Insufficient Marks is one shared wood toast (`Not enough Marks.`), not a line on each chip. A successful buy shows a clearing wood toast — Optic “Shot window looser”, Stock / Barrel “Wobble quieter” — then the plate hides itself. Attack chrome reads `you.shotWindowSec` / `you.wobbleScale` only. A missing feel field stays the bare plate even when a T2 id is equipped.

Headless `HEADLESS_LOOP_OK`, `PART_T2_OK`, and `PART_PEG_OK`. Stills: `artifacts/ux/part_t2_rows.png`, `part_t2_equipped.png`, `part_t2_buy_toast.png`, `part_t2_insufficient.png`, `part_pegs_unowned.png`, `part_pegs_owned.png`.
